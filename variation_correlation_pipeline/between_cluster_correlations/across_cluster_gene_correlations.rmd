...
```{r}
## CURRENT BUG adipose subcutaneous is not in the final table
## not fixed as of 5/26/2021
...
cluster_composition_table<- as_tibble(cluster_composition_table) %>%
  ...
  rename(cur_tis = "tissue", unique_clust = "meta_clusters")
...
composition_plot <- function(dat, tissue_filter,
  ...
  if (summarize_unknown == TRUE) {
    ...
    compress_unknown <- temp_df %>% filter(startsWith(meta_clusters, "unkn")) %>%
                          group_by(cluster_names) %>%
...
composition_plot(cluster_composition_table, "artery_coronary",summarize_all_dupes = TRUE)
```
