...
#####
### Set up environment ###
tabula_key <- read.csv("../general_tissues_tabula_mkh.csv")
gene_location <- "~/work2/tnieuwe1/data/gtex_v8/kendall_gene_clusters"
data_location <- "~/work2/tnieuwe1/data/gtex_v8/tabula_sapiens"
output_locations <- "~/work2/tnieuwe1/data/gtex_v8/output_results/"
#####
...
for (tabula_ind in seq_along(unique_tabula_tissues)) {
    ...
    model_compartment <- model_mat[,1:4]
    model_celltype <- model_mat[,-1:-4]
    ## Run the analysis
    pre_master <- NULL
...
write.csv(master_tables,
          file = paste0(output_locations, "master_tabula_table.csv"),
          row.names = FALSE)