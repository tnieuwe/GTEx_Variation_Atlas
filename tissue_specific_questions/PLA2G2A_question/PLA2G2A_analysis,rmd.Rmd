...
dth_dat <- select(phen_dat, SUBJID, TRISCHD, DTHHRDY)%>%
...
for (pla_clust_ind in seq(nrow(pla_key_dat))) {
    ...
    ## equation
    profile_dat <- read.csv(path_to_profile) %>% select(SAMPID = X,pla_clust = cur_clust) %>%
    ...
    model_res <- rbind(model_res, new_row)
    ## Plot
...
write.csv(joined_dat, "data_out/pla2g2a_DTHHRDY_results.csv", row.names = F)

pdf("data_out/pla2g2a_DTHHRDY_plots.pdf", width = 10, height = 8)
